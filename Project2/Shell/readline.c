...
void readline( int secnum, int i, int argument ) {
	...
	int  t1 = 0;
		...
		lseek (fd, secnum*512, SEEK_SET);
		lseek (fd, 32*i , SEEK_CUR );

		read(fd, name, 8);
		read(fd, ext, 3);
		read(fd, attrib, 2);
		lseek(fd, (secnum*512)+(32*i)+26, SEEK_SET);
		read(fd, cluster, 3);
		lseek(fd, (secnum*512)+(32*i)+24, SEEK_SET);
		read(fd, date, 3);
		lseek(fd, (secnum*512)+(32*i)+22, SEEK_SET);
		read(fd, time, 2);
		lseek(fd, (secnum*512)+(32*i)+28, SEEK_SET);
		read(fd, filesize, 4);
		lseek(fd, 0, SEEK_SET);
		...
		if ( argument == 1 ) {
			if ( isfolder == 1 ) {
				printf( "\t/%s   \t\t\t%2d", name, tcluster);
			} else {
				printf( "\t/%s.%s  \t\t\t%2d", name, ext, tcluster);
			}
		} else {
			if ( isfolder == 1 ) {
				printf( "\n/%s   ", name);
		...
		if ( isfolder == 1 ) {
			for( k = 0; k < 10; k++){
				folderdepth1( 31+tcluster, k, argument, name );
			}
		}
